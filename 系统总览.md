...
| MVP-2 | [002-football-stats-content](specs/002-football-stats-content/) | Stats 对话、Content 赛后报道 | Stats 对话已实现；自动赛后战报 Job 仍偏薄弱 |
...
| 扩展 | [005-player-relationship-analysis](specs/005-player-relationship-analysis/) | TM 履历、双球员关系、时间线/关系图 | **当前 Active**；任务自动化已齐，待 HV-2 收官 |
...
说明：仓库中存在 `StatsStartView.vue`，但**当前 Router 未注册独立 Stats 入口页**；Stats 能力主要通过对话 API（`agentId=stats`）与比赛相关流程承接。
...
- **Content**：完赛后自动生成赛后报道并入 Feed（规格要求）；进度上 Content Job 仍待补齐。
- 数据未就绪时不得猜测比分/事件。
...
|--------|---------|------|
| 1 | 001 Feed MVP | 已签收 |
| 2 | 002 Stats/Content | Progress 未完整收官；Stats 较完整，Content 自动战报偏弱 |
| 3 | 003 Scout/Tactical | 已完成 |
| 4 | 004 Fan/治理 | 已完成（约 2026-07-13） |
| **5** | **005 球员关系分析** | **Active**：T001–T030 自动化任务已勾完，待批次 HV-2 收官 |
...
3. 当前 Active Plan：[`specs/005-player-relationship-analysis/plan.md`](specs/005-player-relationship-analysis/plan.md)
