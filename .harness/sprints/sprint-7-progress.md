...
- [ ] T013 [P] [US2] Add web API client for narrative GET/POST (incl. `force`) in `web/src/api/relationship-narratives.ts` | L1:- L2:- |
- [ ] T014 [P] [US2] Create `RelationshipNarrativePanel` (disabled when analysis not ready; empty CTA; loading; success + AI badge; error/timeout/429 + retry) in `web/src/components/relationship/RelationshipNarrativePanel.vue` | L1:- L2:- |
- [ ] T015 [US2] Integrate narrative panel into relationship page without replacing structured conclusion UI in `web/src/views/RelationshipAnalysisView.vue` | L1:- L2:- |
- [ ] 🚧 **批次7.4门禁: L1 Step4 + 👁 HV-1 (~5 min, 产品/用户)** | 结果: - |
