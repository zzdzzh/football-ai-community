...
8. **Post-Generation Assess Review** (conditional, after spec.md is written and step 7 validation completes):
   ...
   - **If either exists**: execute the full workflow defined in `.harness/assess.md` against the **newly generated or last modified** `SPEC_FILE`:
     - **Target file**: `SPEC_FILE`（本次 `/speckit-specify` 写入或修订的 `spec.md`）
     - **Invocation arguments**: 仅传 `SPEC_FILE` 相对 `projectRoot` 的路径（如 `specs/003-user-auth/spec.md`），由 assess 按文件名自动匹配 Stage2
     - **Do not** call `npx assess run`、外部 LLM 或 `assess.json`；严格按 `.harness/assess.md` 执行（判定阶段 → 准备内容 → 评估计分 → 写报告）
     - **Read-only**: 只审核 `spec.md`，不修改 spec 内容
     - 记录审核结果供 step 9 汇总：`verdict`、`score`、报告路径 `.log/assess/stage-review-*.md`、未通过项摘要
     - 若审核结论为 `fail` 或 `partial`：在 completion 报告中**醒目列出**未通过项与报告路径，提示用户修订 spec 后可重新运行 `/assess` 或再次 `/speckit-specify`
...
# 注意事项
- 对原有文档的修改，必须记录完善的历史行，能够知道每次修改了哪些内容