...
5. **Post-Generation Assess Review** (conditional, after `tasks.md` is written):
   ...
   - **If either exists**: execute the full workflow defined in `.harness/assess.md` against the **newly generated or last modified** `tasks.md`:
     - **Target file**: `FEATURE_DIR/tasks.md`（本次 `/speckit-tasks` 写入或修订的 `tasks.md`）
     - **Invocation arguments**: 仅传 `tasks.md` 相对 `projectRoot` 的路径（如 `specs/003-user-auth/tasks.md`），由 assess 按文件名自动匹配 Stage4
     - **Do not** call `npx assess run`、外部 LLM 或 `assess.json`；严格按 `.harness/assess.md` 执行（判定阶段 → 准备内容 → 评估计分 → 写报告）
     - **Read-only**: 只审核 `tasks.md`，不修改 tasks 内容
     - 记录审核结果供 step 6 汇总：`verdict`、`score`、报告路径 `.log/assess/stage-review-*.md`、未通过项摘要
     - 若审核结论为 `fail` 或 `partial`：在报告中**醒目列出**未通过项与报告路径，提示用户修订 tasks 后可重新运行 `/assess` 或再次 `/speckit-tasks`
